# 2026
## 05.05
这趟在菲律宾潜水的时候，有一句话一致萦绕在我耳边：那不是你人生的问题，那就是你人生的内容。
挺有意思的一句话
## 04.13
最近的在思考不少东西，有投资的，也有生活的。让我意识到，许多认知与底层逻辑是可以跨领域迁移的，比如：
1. 投资中的“分散风险”同样适用于生活——人生的重心不应过度单一，需要建立多元的锚点。
2. 宏观周期的普适性。基于康波周期与美林时钟在A股买入矿业股的逻辑（如紫金矿业、天齐锂业），平移至美股同类公司，也至少能获得一个翻倍的收益。
3. etc.:![img_v3_0210n_f2b5706b-707e-4df6-90bc-f6dde8f578hu](https://github.com/user-attachments/assets/65ae9ff4-f109-4d34-848e-63b0a38c2935)


还在思考一个课题：如果真个过两三年就退休了，我该如何重构生活？剥离了职业属性后，面对漫长的人生难免会有失重感。目前的初步规划如下：
1. 每年国内停留5-6个月。其中2-3个月在家陪伴父母（不过需把握好距离感，避免久居生厌）；其余时间在国内寻找有阳光有山有海的城市旅居。
2. 东南亚：每年3-4个月在东南亚（清迈、岘港等）生活。我还真喜欢东南亚的，阳光充足、人们很友好，还有森林大海
3. 全球旅居：每年安排1-2次、每次1-2个月的跨国旅居（如新西兰房车旅行等等）。
4. 公益和学习：每年投入约1个月参与长期公益项目；并将“每年掌握一项新技能”融入到日常旅途之中。

不过这样的安排是否合理，以及会不会感觉空虚，暂没定论，还得在思考思考。
...
| 护城河 | 影响 | 核心逻辑 |
| :--- | :--- | :--- |
| **1. 习得性界面** | 🏚️ **被摧毁** | 用户多年学习的快捷键和操作语言，被统一的“对话式界面”（Chat）取代，切换成本瞬间归零。 |
| **2. 定制化工作流** | 💨 **蒸发** | 过去需要数年编码的业务逻辑（如法律检索、DCF模型），现在可被领域专家用自然语言写成“Markdown技能文件”，由LLM执行，开发门槛和周期指数级下降。 |
| **3. 公开数据访问** | 🏭 **商品化** | 花费巨资构建的解析器（如解析SEC文件）成为冗余，前沿模型自身就具备理解和解析公开数据的能力。数据本身仍有价值，但“让数据可搜索”这层价值正在崩塌。 |
| **4. 人才稀缺性** | 🔄 **反转** | 过去需要“懂行业的工程师”这一稀缺人才；现在领域专家无需写代码，可直接通过LLM将知识转化为软件，人才瓶颈被打破，进入壁垒降低。 |
| **5. 捆绑销售** | ⚔️ **被削弱** | AI Agent本身就是“终极捆绑者”，它可以自由调度不同工具完成任务，用户无需再为一个捆绑套装支付溢价，解绑成为可能。 |
| **6. 私有和独占数据** | 💪 **更强** | 这是最坚固的护城河之一。无法被复制或合成的独占数据（如实时交易数据、信用评级），在AI时代将变得更有价值，定价权可能反而增强。 |
| **7. 监管与合规锁定** | 🏛️ **结构性护城河** | HIPAA、FDA认证、SOX合规等要求不会因LLM而消失。这些认证和漫长的实施周期构成了强大的切换成本，LLM难以直接撼动。 |
| **8. 网络效应** | 🧲 **依然粘性** | 如Bloomberg的IB chat，价值来自“还有谁在用”。LLM不打破网络效应，甚至可能让流经网络的信息变得更有价值。 |
| **9. 嵌入交易环节** | 💳 **持久** | 直接处理支付、发放贷款的软件，切换意味着中断收入流。LLM可能成为更好的前端，但无法取代底层的交易轨道。 |
| **10. 记录系统地位** | ⏳ **长期受威胁** | 传统软件是数据的权威信源，但AI Agent通过读取邮件、文档等所有信息，正在悄悄构建更完整的用户工作画像，其“记忆”有潜力成为新的信源。 |

#### 三、净效应与风险评估

- **竞争格局剧变：** 竞争从线性增长（3家变4家）变为**组合爆炸（3家变300家）**。大量AI原生创业公司能以更低价格提供核心功能，这才是碾碎定价权的核心力量。
- **转型节奏：** 这是一场**多年转型，而非一夜崩溃**。企业收入有合同锁定，短期不会消失，但市场已提前对护城河瓦解的公司进行估值倍数压缩。

#### 四、真正的威胁：一场“钳形攻击”

1.  **从下方：** 数百个AI原生创业公司涌入，使市场剧烈碎片化。
2.  **从上方：** 水平平台（如Microsoft Copilot）借助AI能力，首次深入垂直领域，将通用工具变成垂直工具。
- **技术栈的颠覆：** “通用Agent框架（SDK）+ 可插拔数据访问（MCP）+ 领域特定技能（Markdown文件）”这一技术栈，让任何水平平台都能轻松进入垂直领域，这是对垂直软件最致命的威胁。

#### 五、风险评估框架与三个检验问题

- **高风险（搜索层）：** 核心价值在于通过界面搜索和访问公开或可授权数据的公司（如传统金融数据终端、法律研究平台）。
- **中等风险（混合组合）：** 同时拥有可防御业务（如独占数据）和暴露业务（如信息检索）的公司。
- **较低风险（监管堡垒）：** 护城河是监管认证、合规基础设施和关键任务工作流深度集成的公司（如医疗电子病历、生命科学平台）。

**【三个检验问题】**
1.  **数据是否独占？** （是→护城河成立；否→价值崩塌）
2.  **是否存在监管锁定？** （是→LLM不改变切换成本；否→主要靠界面维持的切换成本瓦解）
3.  **软件是否嵌入交易？** （是→LLM在你之上，而非取代你；否→你是可替换的）

> **总结：** 垂直SaaS的清算时刻已到。市场正在残酷地区分：哪些公司拥有真正稀缺的、LLM Agent无法触及的东西（如独占数据、监管壁垒、交易网络）。这个对于个人倒也有一定的参考依据
