# 2025
## 1.1
最近一个季度在做一些大模型安全方面的东西，也浅浅的学习了一下这方面的东西，这里来用 SMART 框架来组织一下学习的内容：
****具体性（Specific）****

在大模型安全中，具体性要求明确识别和定义安全目标。例如，针对特定的安全威胁（如越狱攻击、提示注入攻击等），需要制定明确的防护措施和响应策略。这可以包括：
...
可测量性强调对安全目标进行量化评估。对于大模型而言，可以通过以下方式进行测量：

- 监控系统的漏洞检测率
- 测试模型在面对特定攻击时的响应时间
- 评估用户生成内容中的有害信息识别准确率

通过这些指标，可以持续跟踪大模型的安全性能，并及时调整策略。

****可实现性（Achievable）****

可实现性要求设定的目标必须在技术和资源上是可达成的。对于大模型来说，这意味着：

- 确保团队具备必要的技术能力
- 资源配置合理，能够支持持续的安全研究和开发
- 采用合适的技术工具来实施安全措施

例如，利用现有的机器学习框架进行自动化漏洞检测。
...
时限性要求为每个安全目标设定明确的时间框架。这可以帮助团队保持进度并评估成果。例如：

- 设定季度审查机制，以评估安全措施的有效性
- 在特定时间内完成漏洞修复工作

## 大模型自身安全框架

根据最新发布的大模型安全研究报告，大模型面临多种安全风险，包括训练数据质量、算法鲁棒性、系统平台安全等方面的问题。为此，报告提出了一个涵盖四个层面的自身安全框架，包括：

1. **安全目标**：明确需要达成的具体安全成果。
2. **安全属性**：定义系统应具备的基本安全特征，如机密性、完整性和可用性。
3. **保护对象**：识别需要保护的信息资产和系统组件。
4. **安全措施**：制定针对识别出的风险采取的具体防护措施，包括技术手段和管理策略。
